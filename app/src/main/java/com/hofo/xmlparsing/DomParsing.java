 ...
 */
public class DomParsing extends BaseActivity {
    ...
    public void parsing() throws Exception {
        ...
        Document document = builder.parse(is);//解析数流：通过解析器拿到数流
        //DOM解析的原理：1、获取文档的标签(跟标签)
        Element root = document.getDocumentElement();
        log("根节点:" + root.getTagName());
        //2、获取文档的标签下所有的子节点
        listAllChildNodes(root, 0);// 参数0表示设定根节点层次为0,它的前面不打印空格.
    }
            ...
            if (node.hasChildNodes()) {
                ...
                for (int i = 0; i < nodelist.getLength(); i++) {
                    ...
                    if (nodelist.item(i).getNodeType() == Node.TEXT_NODE
                        ...
                        log(nodelist.item(i).getTextContent());// 在开始标签后面添加文本内容
